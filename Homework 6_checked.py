...
""" Լիանա ջան, խնդիրները պետք է ֆունկցիաներով լուծեիք։ Եթե անհասկանալի բան կա դրանց հետ կապված, հարցրեք անպայման։"""
...
'''lst = [elem ** 2 for elem in range(1, 31)]
print(lst)'''
...
def example_list (4, 1, 2253, 32, 13, 64, 1, 90):
x = []
for i in example_list:
    x.sum(i += 1)
    return x
b = 13
    if b in example_list:
    break
print(x)'''
...
b = lambda r, pi = 3.14: pi * r ** 2
print(b(3))'''

'''def sum_to_power(x, y, p):
    return (x + y) ** p
c = lambda x, y, p: (x + y) ** p
print(c(2,3,4))'''
...
nums = list(filter(lambda x: x % 2 == 7, lst_1 ))
print(nums)'''
